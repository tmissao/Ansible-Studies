...
Dynamic Inventory is an Ansible's feature which is possible to execute a file and use its return as inventories

In order to create a dynamic inventory the executable file should:

- `Be an executable file` - It can be written in any language, and must be executed from command line
- `Accepts the command line options --list and --host`
- `Returns JSON dictionary when used with --list`
- `Returns JSON dictionary when used with --host`
...
```bash
# ansible <target> -i <script-for-dynamic-inventory>
